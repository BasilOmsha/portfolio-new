import type { ProjectChapterProps } from '@/constants'

function ProjectChapter({
    index,
    title,
    className,
    imageSrc,
    imageAlt,
    techCount,
    totalDuration,
    description,
    technologies,
    links = []
}: ProjectChapterProps) {
    return (
        <div className="projects-chapter-container">
            <div className="Illustration">
                <div className="radial-glow"></div>
                <img className="lazy is-loaded" src={imageSrc} alt={imageAlt} />
            </div>
            <div className={`project-chapter ${className}`} data-index={index}>
                <h3 className="section-title element delay-1">{title}</h3>
                <div className="information element delay-3">
                    {techCount} — {totalDuration}&nbsp;
                    {/* <span className="optional"> of </span> */}
                </div>
                <div className="description element delay-2">
                    <p dangerouslySetInnerHTML={{ __html: description }} />
                </div>
                {links.length > 0 && (
                    <div className="project-links element delay-3">
                        {links.map((link, index) => {
                            const isClickable = link.enabled !== false

                            const content = (
                                <>
                                    {link.type === 'github' ? (
                                        <svg
                                            width="20"
                                            height="20"
                                            viewBox="0 0 24 24"
                                            fill="none"
                                            xmlns="http://www.w3.org/2000/svg"
                                        >
                                            <path
                                                d="M12 0C5.374 0 0 5.373 0 12 0 17.302 3.438 21.8 8.207 23.387c.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23A11.509 11.509 0 0112 5.803c1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576C20.566 21.797 24 17.3 24 12c0-6.627-5.373-12-12-12z"
                                                fill="currentColor"
                                            />
                                        </svg>
                                    ) : (
                                        <svg
                                            width="20"
                                            height="20"
                                            viewBox="0 0 24 24"
                                            fill="none"
                                            xmlns="http://www.w3.org/2000/svg"
                                        >
                                            <path
                                                d="M12 2L2 7L12 12L22 7L12 2Z"
                                                stroke="currentColor"
                                                strokeWidth="2"
                                                strokeLinecap="round"
                                                strokeLinejoin="round"
                                            />
                                            <path
                                                d="M2 17L12 22L22 17"
                                                stroke="currentColor"
                                                strokeWidth="2"
                                                strokeLinecap="round"
                                                strokeLinejoin="round"
                                            />
                                            <path
                                                d="M2 12L12 17L22 12"
                                                stroke="currentColor"
                                                strokeWidth="2"
                                                strokeLinecap="round"
                                                strokeLinejoin="round"
                                            />
                                        </svg>
                                    )}
                                    {link.label}
                                </>
                            )

                            return isClickable ? (
                                <a
                                    key={index}
                                    href={link.url}
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className={`project-link ${link.type === 'github' ? 'github-link' : 'deploy-link'}`}
                                >
                                    {content}
                                </a>
                            ) : (
                                <span
                                    key={index}
                                    className={`project-link ${link.type === 'github' ? 'github-link' : 'deploy-link'} disabled`}
                                    style={{ cursor: 'default', opacity: 0.7 }}
                                >
                                    {content}
                                </span>
                            )
                        })}
                    </div>
                )}
                <div className="projects">
                    {technologies.map((project, index) => (
                        <div
                            key={project.number}
                            className={`project element delay-${4 + index}`}
                            // href={project.href}
                            data-router-disabled=""
                        >
                            <div className="column project-number">{project.number}</div>
                            <div className="column project-title-badges">
                                <div className="title">{project.title}</div>
                                <div className="badges">
                                    {project.badges.map((badge) => (
                                        <span key={badge} className={`badge is-${badge}`}>
                                            {badge}
                                        </span>
                                    ))}
                                </div>
                            </div>
                            <div className="column project-duration">{project.component}</div>
                        </div>
                    ))}
                </div>
            </div>
        </div>
    )
}

export default ProjectChapter
